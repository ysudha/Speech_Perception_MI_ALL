...
for ipat = 1:num_patients % for each patient, run the loop
    ...
    % figire out normalization value for making the maximum value of window
    % 1, of VOT 123 of each patient =1
    filename = sprintf('di_matrix_subj%d_VOT123_mem2K4down15ver2.mat', ipat);
    temp = load(filename);
    norm_const = max(max(temp.di_matrix{1,1}))
...
%% look at temporal to frontal in each time window
