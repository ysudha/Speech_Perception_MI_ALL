% contrast vector plotted with yellow squarees for healthy  (instead of fig 3)
...
%% paper figure for plotting: comparing mid, low and high VOTs, average of healthy and aphasia guy
...
figure; hold on;
plot(ntim,(aphasia_low_VOT_energy),'-.d','LineWidth',6,'MarkerSize',4,'Color',[27,158,119]/255);hold on;
plot(ntim,(aphasia_high_VOT_energy),'-.s','LineWidth',6,'MarkerSize',4,'Color',[117,112,179]/255);hold on;
plot(ntim,(aphasia_mid_VOT_energy),'-.o','LineWidth',6,'MarkerSize',4,'Color',[217,95,2]/255);hold on;
...
plot(ntim,mean(healthy_mid_VOT_energy),'-o','LineWidth',4,'MarkerSize',4,'Color',[217,95,2]/255);hold on;
hold on;
legend('LowVOT - Aphasic','HighVOT - Aphasic','MidVOT - Aphasic','LowVOT - Healthy','HighVOT - Healthy','MidVOT - Healthy','Location','bestoutside');
legend box off
set(gcf,'color','w');
xlabel('Time (ms)','FontSize',16);
ylabel('Frobenius norm','FontSize',16);
ax=gca;
ax.FontSize = 16;
...
%% plotting the contrast vector differently
...
ylabel('Contrast vector','FontSize',20);
ax=gca;
ax.FontSize = 16;

cluster_pvalues = zeros(1,length(ntim));
for isig = 1:length(sig_cluster_starts)
    cluster_pvalues(sig_cluster_starts(isig):sig_cluster_end(isig))=1;
end


for i = 1:length(ntim)
    patstats = cluster_pvalues;
    if(patstats(i)==1)
        a=patch([ntim(i)-0.5*10 ntim(i)+0.5*10 ntim(i)+0.5*10 ntim(i)-0.5*10], [yl(2)-0.02 yl(2)-0.02 yl(2)-0.005 yl(2)-0.005],'y')
        a.Annotation.LegendInformation.IconDisplayStyle = 'off';

        alpha(0.3);
        str = sprintf('*');
        text(ntim(i)-3,yl(2)-0.001,str,'FontSize',20);
        a.Annotation.LegendInformation.IconDisplayStyle = 'off';
    end
end