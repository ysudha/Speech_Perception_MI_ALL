% this program was written to see if the difference in energy in S is due
% to differences in energy of X< or beacause of the decompostion? Turns
% out, since L is fixed for each patient, X just has more energy than S. 
...
parfor ipat = 1:np
    ...
    for iwin = 1:nwin
        ...
        [U1,S1,V1] = eig(squeeze(X(:,:,iwin)));
        energy_Xtime(ipat,iwin) = sum(abs(diag(S1)));
    end
    S_pat{1,ipat} = S;
    L_pat{1,ipat} = L;
    toc
