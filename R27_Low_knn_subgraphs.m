...
% 50 ms windows, 13 windows, 3 baseline, 11 main windows.
...
%G = graph(empty_graph);%,chnames);
plot(G,'XData',cell2mat(chanNamesLocs(chlist,2)),'YData',cell2mat(chanNamesLocs(chlist,3)),'NodeFontSize',14)
az=-90;
ele = 90;
view([az ele])
box off;
...
healthy_matrices = squeeze(sum(diff_lowVOT(1:np-1,:,:,:)));
aphasic_matrices = squeeze((diff_lowVOT(np,:,:,:)));
