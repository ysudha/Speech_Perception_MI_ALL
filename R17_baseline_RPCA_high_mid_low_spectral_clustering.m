...
% (3) load graphs, edge = highVOT ...looking at increases and decreases
 ...
 %% (1) load graphs, edge = BSmidVOT - BSlowVOT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%Robust PCA in time%%%%%%%%%%%%%%%%%%
...
clusters_mid_low  = zeros(np,nch,nwin-2);
for ipat = 1:np
    ...
    for iwin = 3:nwin
      ...
      % mat(mat<0)=0;
       clusters_mid_low(ipat,:,iwin) ...%spectralcluster(mat,k,'Distance','precomputed','LaplacianNormalization','symmetric');
       = spectralcluster(mat,k,'Distance','minkowski','P',3);
       [~,ind] = sort(squeeze(clusters_mid_low(ipat,:,iwin)));
       diff_mid_lowVOT(ipat,:,:,iwin-2) = mat(ind,ind);
