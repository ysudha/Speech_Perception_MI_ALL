...
for ipat = 1:num_patients % for each patient, run the loop
    ...
    [normalizing_val ~] = max(max(thresh.di_matrix{3,1} - thresh.di_matrix{1,1}));
    ...
    for nt = 1:nwin % calculate degrees for each time window
        mat = (di_matrix{nt,1} - di_matrix{1,1})./normalizing_val;
        mat(mat<0)=0;
...
% lets look at the average of all patients data, increase in DI from
% baseline

mat1= zeros(30,43); mat2=zeros(30,43); mat3=zeros(30,43);
for ipat = 1:num_patients
    temp = node_indeg{1,ipat};
    mat1 = mat1+ squeeze(temp(1,:,:));
    mat2 = mat2+ squeeze(temp(2,:,:));
    mat3 = mat3+ squeeze(temp(3,:,:));
end
figure;
subplot(1,3,1); imagesc(mat1); colorbar; colormap jet;
subplot(1,3,2); imagesc(mat2); colorbar; colormap jet;
subplot(1,3,3); imagesc(mat3); colorbar; colormap jet;
