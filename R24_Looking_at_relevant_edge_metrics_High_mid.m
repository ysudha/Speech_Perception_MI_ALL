...
diff_mid_highVOT(diff_mid_highVOT>0)=0;
diff_mid_highVOT = abs(diff_mid_highVOT);

for ipat = 1:np
    figure;
    mat = squeeze(diff_mid_highVOT(ipat,:,:,1));
...
for i = 1:41
    ...
    for ipat = 1:np
        ...
        mat = squeeze(diff_mid_highVOT(ipat,:,:,i));
        bar(ipat,norm(mat,'fro'));
...
for iwin = 1:41
    ...
    mask = ones(nch,nch);
    for ipat = 1:np-1
        S = squeeze(diff_mid_highVOT(ipat,:,:,iwin));
        Sap = squeeze(diff_mid_highVOT(22,:,:,iwin));
        mask_subj_specific = Sap>S;
        mask = mask.*mask_subj_specific;
    end
    imagesc(mask)
