...
chlist = [1:25 28];
miVOT123 = load('mi_matrix_VOT123_20ms_win.mat'); %with mem 2
np = 22; %number of people 21 undergrads, 1 aphasia
nwin = 35;
nch = length(chlist);
ntim = [[1:10:91] [101:10:341]];
ntim = ntim*2-202;
...
for ipat = 1:np
    ...
    for iwin = 11:nwin
        baseline = (mi123{1,1}  )./1;
        mat2 = squeeze(mi123{iwin,1} - baseline);
        mat = squeeze(mat2);
        diff_lowVOT(ipat,:,:,iwin-10) = mat(chlist,chlist);
    end
...
figure;
for i = 1:np-1
    plot(ntim(11:end),low_VOT_energy(i,:),'b-*','LineWidth',4);
    hold on;
end
plot(ntim(11:end),low_VOT_energy(np,:),'r-*','LineWidth',4);
...
figure;
imagesc(ntim(11:end), 1:22,stats_low_gt_bs);
