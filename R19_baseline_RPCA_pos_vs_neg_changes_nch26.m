...
stats_low_gt_mid = zeros(np,nwin);
stats_high_gt_mid = zeros(np,nwin);
%% plotting code (1) 
...
for ipat = 1:np
    ...
    stats_m_gt_h(ipat) = sum(stats<0.05);
end
%%%%%%%%%
figure; 
plot(X2(1:end-1,:)','-b*','LineWidth',4);hold on;
...
for ipat = 1:np
    ...
    fprintf('%d,%s\n',ipat, num2str(stats<0.05));
    stats_high_gt_mid(ipat,:) = stats<0.05;
    stats_m_ls_h(ipat) = sum(stats<0.05);
end
%%%%%%%%%%%%%%
%% plotting (2)
...
for ipat = 1:np
    ...
    fprintf('%d,%s\n',ipat, num2str(stats<0.05));
    stats_m_ls_l(ipat) = sum(stats<0.05);
    stats_low_gt_mid(ipat,:) = stats<0.05;
...
%    fprintf('%d,%s\n',ipat, num2str(stats<0.05));
    stats_m_gt_l(ipat) = sum(stats<0.05);
...
for ipat = 1:np
    ...
    stats_l_ls_h(ipat) = sum(stats<0.05);
end
%%%%%%%%%
figure; 
plot(X2(1:end-1,:)','-b*','LineWidth',4);hold on;
...
for ipat = 1:np
    ...
    stats_l_gt_h(ipat) = sum(stats<0.05);
...
bar(stats_l_gt_h);
title('Edge metric: lowVOT - HighVOT >0');
plot(1:np,stats_l_gt_h,'ko')
subplot(3,1,2);hold on;
bar(stats_m_gt_l);
plot(1:np,stats_m_gt_l,'ko')
title('Edge metric: midVOT - LowVOT >0');
subplot(3,1,3);hold on;
bar(stats_m_gt_h);
plot(1:np,stats_m_gt_h,'ko')
title('Edge metric: midVOT - HighVOT >0');

figure;
subplot(3,1,1);hold on;
bar(stats_l_ls_h);
plot(1:np,stats_l_ls_h,'ko');
title('Edge metric: HighVOT-lowVOT >0');
subplot(3,1,2);hold on;
bar(stats_m_ls_l);
plot(1:np,stats_m_ls_l,'ko')
title('Edge metric: LowVOT - midVOT >0');  % this is significant
subplot(3,1,3);hold on;
bar(stats_m_ls_h);
plot(1:np,stats_m_ls_h,'ko')
title('Edge metric: HighVOT -MidVOT>0');   % this is significant

figure;
imagesc(stats_high_gt_mid);
title('High-mid>0');
figure;
imagesc(stats_low_gt_mid);
title('Low - mid>0');
